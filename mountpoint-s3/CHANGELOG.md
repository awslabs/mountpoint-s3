...
* `--read-only` may now be used with a FUSE file descriptor mount point (`/dev/fd/N`). Previously this combination was rejected, since Mountpoint does not perform the `mount` syscall in that case and so cannot apply the option itself. It is now accepted as a statement that the caller performed the mount read-only, which Mountpoint enforces for the operations it serves. ([#1939](https://github.com/awslabs/mountpoint-s3/pull/1939) by @yerzhan7)
* Mountpoint now enforces `--read-only` itself, refusing file system operations that would modify the mount with `EROFS`. Previously a read-only mount relied entirely on the kernel to reject them, which it still does for a directory mount point; this makes the behaviour the same when the mount point is a FUSE file descriptor and Mountpoint did not perform the mount. ([#1939](https://github.com/awslabs/mountpoint-s3/pull/1939) by @yerzhan7)
