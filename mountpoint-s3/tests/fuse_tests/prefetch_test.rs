...
fn prefetch_test_etag<F>(
    ...
    F: FnOnce(&str, TestSessionConfig) -> (TempDir, BackgroundSession, TestClientBox),
{
    let object_size = initial_read_window_size * 2;
...
#[test_case(256 * 1024, 500 * 1024; "first request size smaller than first block read size")]
...
fn prefetch_test_etag_mock_with_cache(initial_read_window_size: usize, read_size: usize) {
    let part_size = 256 * 1024;
    prefetch_test_etag(
        fuse::mock_session::new_with_cache(InMemoryDataCache::new(1024)),
        "prefetch_test_etag_mock",
        part_size,
        initial_read_window_size,
        read_size,
...
#[cfg(feature = "s3_tests")]
#[test_case(8 * 1024 * 1024, 1024; "default first request size, much larger than first block read size")]
#[test_case(10 * 1024 * 1024, 1024; "first request size greater than default,  much larger than first block read size")]
#[test_case(8 * 1024 * 1024, 10 * 1024 * 1024; "first request size smaller than first block read size")]
