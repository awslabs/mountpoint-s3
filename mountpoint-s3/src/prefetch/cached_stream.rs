...
#[derive(Debug)]
pub struct CachedPartStream<Client, Cache, Runtime> {
    inner: CachedPartStreamInner<Client, Cache>,
    runtime: Runtime,
}

impl<Client, Cache, Runtime> CachedPartStream<Client, Cache, Runtime> {
    pub fn new(client: Arc<Client>, runtime: Runtime, cache: Cache) -> Self {
        Self {
            inner: CachedPartStreamInner {
                client,
                cache: Arc::new(cache),
            },
            runtime,
...
impl<Client, Cache, Runtime> ObjectPartStream for CachedPartStream<Client, Cache, Runtime>
where
    ...
    Runtime: Spawn,
{
    ...
    fn spawn_get_object_request(
    ...
    ) -> RequestTask<Self::ClientError> {
        let range = get_aligned_request_range(range, self.inner.cache.block_size());
        ...
        let request_task = {
            ...
            let etag = if_match.clone();
            let span = debug_span!("prefetch", ?range);

            async move {
                inner
                    .get_from_cache(&bucket, &key, etag, range, part_queue_producer)
                    .await;
            }
            .instrument(span)
        };
...
#[derive(Debug)]
struct CachedPartStreamInner<Client, Cache> {
    client: Arc<Client>,
    cache: Arc<Cache>,
}

impl<Client, Cache> Clone for CachedPartStreamInner<Client, Cache> {
    fn clone(&self) -> Self {
        Self {
...
impl<Client, Cache> CachedPartStreamInner<Client, Cache>
where
    Client: ObjectClient + Send + Sync + 'static,
    Cache: DataCache + Send + Sync,
{
    ...
    fn update_cache(&self, cache_key: &CacheKey, block_index: u64, block: &ChecksummedBytes) {
        ...
        match self.cache.put_block(cache_key.clone(), block_index, block.clone()) {
            ...
            Err(error) => {
                warn!(key = cache_key.s3_key, block_index, ?error, "failed to update cache");
            }
        };
    }
}
...
mod tests {
    ...
    fn test_read_from_cache(
        ...
        let stream = CachedPartStream::new(mock_client.clone(), runtime, cache);
        ...
        let first_read_count = {
            ...
            let request_task = stream.spawn_get_object_request(bucket, key, etag.clone(), range, 0);
