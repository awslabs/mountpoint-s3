    ...
    Stream: ObjectPartStream,
{
    ...
    pub fn new(part_stream: Stream, config: PrefetcherConfig) -> Arc<Self> {
        Arc::new(Self { part_stream, config })
    }

    /// Start a new get request to the specified object.
    pub fn get(self: &Arc<Self>, bucket: &str, key: &str, size: u64, etag: ETag) -> PrefetchGetObject<Stream> {
        PrefetchGetObject::new(self.clone(), bucket, key, size, etag)
    }
}
...
#[async_trait]
impl<Stream> crate::store::PrefetchGetObject for PrefetchGetObject<Stream>
where
    Stream: ObjectPartStream + Send + Sync + 'static,
{
    type ClientError = Stream::ClientError;
...
impl<Stream> PrefetchGetObject<Stream>
where
    Stream: ObjectPartStream,
{
    /// Create and spawn a new prefetching request for an object
    fn new(inner: Arc<Prefetcher<Stream>>, bucket: &str, key: &str, size: u64, etag: ETag) -> Self {
        PrefetchGetObject {
            inner: inner.clone(),
            current_task: None,
    ...
    fn spawn_next_request(&mut self) -> Option<RequestTask<Stream::ClientError>> {
        ...
        let task = self.inner.part_stream.spawn_get_object_request(
            &self.bucket,
            &self.key,
            self.etag.clone(),
            range,
...
mod tests {
    ...
    /// Wrapper for `Prefetcher<Stream>` objects with `Stream::ClientError==E`. Allows to dynamically use
    /// prefetchers built with different `ObjectPartStream` implementations.
    struct PrefetcherBox<E> {
        get_fn: Box<GetObjectFn<E>>,
    }

    impl<E> PrefetcherBox<E>
    where
        E: std::error::Error + Send + Sync + 'static,
    {
        fn new<Stream>(part_stream: Stream, config: PrefetcherConfig) -> Self
        where
            Stream: ObjectPartStream<ClientError = E> + Send + Sync + 'static,
        {
            let prefetcher = Prefetcher::new(part_stream, config);
            PrefetcherBox {
                get_fn: Box::new(move |bucket: &str, key: &str, size: u64, etag: ETag| {
                    Box::new(prefetcher.get(bucket, key, size, etag))
                }),
            }
        }

        fn get(&self, bucket: &str, key: &str, size: u64, etag: ETag) -> Box<dyn PrefetchGetObject<ClientError = E>> {
            (self.get_fn)(bucket, key, size, etag)
        }
    }
