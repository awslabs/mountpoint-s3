...
mod tests {
    ...
    #[test_case(["_", "s3://demo_s3_bucket/prefix",                                    "/mnt/test", "-o", "ro"].to_vec(),                    true,  Err(S3PathError::PrefixError(PrefixError::MissingFinalDelimiter)))]
    #[test_case(["_", "s3://demo_s3_bucket/prefix/",                                   "/mnt/test", "-o", "ro,prefix=foo/"].to_vec(),        true,  Err(S3PathError::MultiplePrefixes))]
    #[test_case(["_", "s3://arn:aws:s3:::demo_s3_bucket",                              "/mnt/test", "-o", "ro"].to_vec(),                    true,  Err(S3PathError::InvalidBucketNameS3URI))]
    #[test_case(["_", "s3://arn:aws:s3:::demo_s3_bucket/prefix",                       "/mnt/test", "-o", "ro"].to_vec(),                    true,  Err(S3PathError::PrefixError(PrefixError::MissingFinalDelimiter)))]
    #[test_case(["_", "s3://arn:aws:s3:region:account-id:accesspoint/my-access-point", "/mnt/test", "-o", "ro"].to_vec(),                    true,  Err(S3PathError::PrefixError(PrefixError::MissingFinalDelimiter)))]
    #[test_case(["_", "s3://arn:aws:s3::123456789012:accesspoint/mfzwi23gnjvgw.mrap",  "/mnt/test", "-o", "ro"].to_vec(),                    true,  Err(S3PathError::PrefixError(PrefixError::MissingFinalDelimiter)))]
    fn test_fstab_cli_args_parses(args: Vec<&str>, should_parse: bool, expected_prefix: Result<String, S3PathError>) {
        ...
        if let Ok(cli_args) = res {
            match expected_prefix {
                ...
                Err(expected_err) => {
                    let actual_err: S3PathError = cli_args
                        .s3_path()
    ...
    fn test_fstab_cli_args_fails_multiple_prefixes() {
        ...
        assert!(format!("{:#}", err).contains("multiple prefixes supplied"), "{:#}", err)
    }
