...
mod tests {
    ...
    impl TestContext {
        fn new() -> Self {
            INIT.call_once(|| {
                ...
                RECORDER.set(recorder).unwrap();
            });

            let recorder = RECORDER.get().unwrap().clone();
            TestContext { recorder }
        }
    ...
    fn test_fuse_write_metrics() {
        ...
        let test_session = mock_session::new("write_metrics", config);
        ...
        assert_metric_exists(&context.recorder, "fuse.total_bytes", &[("type", "write")]);
    ...
    fn test_fuse_read_metrics() {
        ...
        let test_session = mock_session::new("read_metrics", config);
        let path = test_session.mount_path().join("test.txt");
        ...
        let read_handle = get_metric(&context.recorder, "fs.current_handles", &[("type", "read")]);
        assert!(read_handle.gauge() >= 1.0, "should have at least 1 read handle");
    ...
    #[test]
    fn test_fuse_request_metric_errors() {
        let context = setup();

        let test_session = mock_session::new("error_metrics", TestSessionConfig::default());
    ...
    fn verify_common_metrics(context: TestContext) {
        // Mostly sanity checks for other requests we expect
        for request in ["open", "getattr", "fsync"] {
            assert_metric_exists(&context.recorder, FUSE_REQUEST_LATENCY, &[(ATTR_FUSE_REQUEST, request)]);
        }

        assert_metric_exists(&context.recorder, "fs.inodes", &[]);
        assert_metric_exists(&context.recorder, "fs.inode_kinds", &[("kind", "file")]);
        assert_metric_exists(&context.recorder, "fs.inode_kinds", &[("kind", "directory")]);
        assert_metric_exists(&context.recorder, "fuse.op_unimplemented", &[("op", "getxattr")]);
    }

    #[test]
    fn test_random_access_triggers_out_of_order() {
        let context = setup();

        let test_session = mock_session::new("random_test", TestSessionConfig::default());
        ...
        let initial_out_of_order = context
            .recorder
        ...
        assert!(
            final_out_of_order > initial_out_of_order,
            "Random access should increment out_of_order metric"
        );
    }
}
