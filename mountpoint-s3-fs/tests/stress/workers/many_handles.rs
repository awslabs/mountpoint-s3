...
impl Worker for ManyHandles {
    ...
    /// The only in-process I/O buffer is the small round-robin touch buffer; report it so the
    /// peak-RSS invariant excludes memory a real out-of-process application would own.
