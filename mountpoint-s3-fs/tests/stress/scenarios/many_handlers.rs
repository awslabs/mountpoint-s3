...
/// A hoarder opens `HANDLES_PER_WORKER` handles before its first round-robin read, which can take
/// well over the 20s default under memory pressure; the readers keep the default ceiling.
fn max_idle(worker: &dyn Worker) -> Duration {
    match worker.kind() {
        "many_handles" => Duration::from_secs(60),
        _ => default_max_idle(worker),
    }
}
