//! `mixed_rw_read8_write16`: like `mixed_rw` (16 readers + 24 writers under the 512 MiB memory
//! limit), but reads use an 8 MiB part size and writes use a 16 MiB part size — the reverse of
//! `mixed_rw_read16_write8`. The two land in *separate* `PagedPool` size-pools, so read and write
//! buffers can never be reused for each other and each size-pool fragments independently against
//! the shared budget.
...
const READ_PART_SIZE: usize = 8 * 1024 * 1024; // 8 MiB reads
const WRITE_PART_SIZE: usize = 16 * 1024 * 1024; // 16 MiB writes
const WRITE_OBJECT_SIZE: usize = 100 * 1024 * 1024; // 100 MiB

#[test]
fn mixed_rw_read8_write16() {
