...
pub mod s3_session {
    ...
    fn new_with_test_client_inner(
        ...
        // its own size-pool (identical sizes dedupe to one, keeping the prunable entry). Reads are
        // prefetch buffers (prunable, so the limiter keeps a reserve for them); write buffers hold
        // dirty data and are not reclaimable, so they are non-prunable.
        let read_part_size = test_config.effective_read_part_size();
        let pool = PagedPool::config()
            .with_candidate_sizes([
                CandidateSize::prunable(test_config.cache_block_size),
                CandidateSize::prunable(read_part_size),
                CandidateSize::new(test_config.part_size),
            ])
            .with_memory_limit(test_config.mem_limit)
            .build();
