...
impl InodeHandleMap {
    ...
    ///
    ///  Will fail if there is another active writer or more than one active reader.
    pub fn set_writer(&self, locked_inode: &InodeLockedForWriting<'_>, fh: u64) -> Result<(), SetWriterError> {
        let mut handles = self.handles.lock().unwrap();
        let entry = handles.entry(locked_inode.ino).or_default();
        // TODO(Jonathon): return back to `> 0` when github.com/google/gvisor/issues/10385 is
        // resolved. gVisor's gofer client opens a read-only "control FD" on every dentry before
        // any write open, so a single active reader is expected and must not block writes.
        let active_readers = entry
            .readers
            .values()
            .filter(|state| **state == HandleState::Active)
            .count();
        if active_readers > 1 {
