...
use crate::memory::{BufferKind, PagedPool, PoolBuffer};
use crate::sync::Arc;
...
impl UploadBuffer {
    ...
    fn new(
        ...
        let data = ReservedBuffer::new(capacity, pool, mem_limiter);
        Ok(Self { data, hasher })
    }
    ...
    fn try_new(
        ...
        let hasher = ChecksumHasher::new(checksum_algorithm)?;
        if let Some(data) = ReservedBuffer::try_new(capacity, pool, mem_limiter) {
    ...
    fn write<'a>(&mut self, slice: &'a [u8]) -> Result<&'a [u8], ChecksumHasherError> {
        let (written, remaining) = self.data.write(slice);
    ...
    fn freeze(self) -> Result<(Bytes, Option<UploadChecksum>), ChecksumHasherError> {
        Ok((self.data.into_bytes(), self.hasher.finalize()?))
    }
}
...
impl ReservedBuffer {
    /// Force creating a new buffer regardless of available memory
    fn new(capacity: usize, pool: &PagedPool, mem_limiter: Arc<MemoryLimiter>) -> Self {
        mem_limiter.reserve(BufferArea::Upload, capacity as u64);
        Self {
            buffer: pool.get_buffer(capacity, BufferKind::Upload),
            len: 0,
            mem_limiter,
        }
    }
    ...
    /// Try creating a new buffer, return `None` if unable to reserve memory for a buffer with the given capacity
    fn try_new(capacity: usize, pool: &PagedPool, mem_limiter: Arc<MemoryLimiter>) -> Option<Self> {
    ...
    /// Write a slice to the buffer. The slice will be trimmed to fit into the buffer.
    /// Returns (slice_written, slice_remainining).
    fn write<'a>(&mut self, slice: &'a [u8]) -> (&'a [u8], &'a [u8]) {
        let available_cap = self.buffer.capacity() - self.len;
        let (left, right) = slice.split_at(available_cap.min(slice.len()));
        let new_len = self.len + left.len();
        self.buffer.as_mut()[self.len..new_len].copy_from_slice(left);
        self.len = new_len;
        (left, right)
    }
