...
// which is a common IO size. But Linux's readahead will try to read an extra 256k on on
...
// send us a READ request for that 256k, so we'll have to block waiting for it even if
// the application doesn't want it. This is all in the noise for sequential IO, but
// waiting for the readahead hurts random IO. So we add 256k to the first request size
...
pub const INITIAL_READ_WINDOW_SIZE: usize = 1024 * 1024 + 256 * 1024;
