    ...
    pub fn new(buffer_sizes: impl Into<Vec<usize>>) -> Self {
        ...
        let size_pools = buffer_sizes
            ...
            .map(|&buffer_size| SizePool {
                ...
                empty_page_count: AtomicUsize::new(0),
                buffer_size,
            })
            .collect();
...
impl PagedPoolInner {
    fn notify_page_empty_for_size(&self, size: usize) {
        let Some(pool) = self.get_pool_for_size(size) else {
            return;
        };
        pool.empty_page_count.fetch_add(1, Ordering::SeqCst);
    }
    ...
    fn get_pool_for_size(&self, size: usize) -> Option<&SizePool> {
        ...
        let index = self.size_pools.partition_point(|p| p.buffer_size < size);
    ...
    fn trim(&self) -> bool {
        ...
        for pool in &self.size_pools {
            if pool.empty_page_count.load(Ordering::SeqCst) == 0 {
                continue;
            }
            ...
            if pages_freed > 0 {
                tracing::trace!(size = pool.buffer_size, pages_freed, "free empty memory pool pages");
                removed = true;
            }
            metrics::histogram!("pool.trim", "size" => format!("{}", pool.buffer_size)).record(pages_freed as f64);
        }
...
struct SizePool {
    ...
    empty_page_count: AtomicUsize,
    buffer_size: usize,
}

impl SizePool {
    fn reserve(&self, pool: &PagedPool) -> Option<PagedBufferPtr> {
        {
        ...
        tracing::trace!(size = self.buffer_size, "allocate new memory pool page");
        let page = Page::new(self.buffer_size, pool);
        let (buffer_ptr, _) = page.try_reserve().unwrap();
        write_pages.push(page);
        Some(buffer_ptr)
    }

    fn try_get_buffer_ptr<'a>(&self, pages: impl Iterator<Item = &'a Page>) -> Option<PagedBufferPtr> {
        for page in pages {
            if let Some((buffer_ptr, page_status)) = page.try_reserve() {
                if let PageStatus::Empty = page_status {
                    self.empty_page_count.fetch_sub(1, Ordering::SeqCst);
                };
                return Some(buffer_ptr);
            }
        }
...
impl Page {
    ...
    fn new(buffer_size: usize, pool: &PagedPool) -> Self {
        assert_ne!(buffer_size, 0);
        let layout = Layout::array::<[u8; Self::BUFFERS_PER_PAGE]>(buffer_size).unwrap();
        ...
        let last_offset = unsafe { bytes.add((Self::BUFFERS_PER_PAGE - 1) * buffer_size) };
        let inner = PageInner {
            bytes,
            last_offset,
    ...
    fn try_reserve(&self) -> Option<(PagedBufferPtr, PageStatus)> {
        let (index, page_status) = consume(&mut self.inner.free_bitmask.lock().unwrap())?;
        let offset = index * self.inner.buffer_size;
    ...
    fn release(&self, ptr: *mut u8) {
        ...
        let index = offset as usize / self.inner.buffer_size;
        let mask = !(1u16 << index);
        let mut bitmask = self.inner.free_bitmask.lock().unwrap();
        *bitmask &= mask;
        if *bitmask == 0 {
            if let Some(pool) = self.inner.pool.upgrade() {
                pool.notify_page_empty_for_size(self.inner.buffer_size);
            }
        }
    }
