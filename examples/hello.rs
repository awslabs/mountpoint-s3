...
use std::os;
use fuse::{Filesystem, Request, FuseResult, fuse_attr, fuse_entry_out, fuse_attr_out, DirBuffer};
...
impl Filesystem for HelloFS {
	fn lookup (&mut self, _req: &Request, parent: u64, name: &PosixPath) -> FuseResult<fuse_entry_out> {
	...
	fn getattr (&mut self, _req: &Request, ino: u64) -> FuseResult<fuse_attr_out> {
		match ino {
	...
	fn read (&mut self, _req: &Request, ino: u64, _fh: u64, offset: u64, _size: uint) -> FuseResult<Vec<u8>> {
		if ino == 2 {
	...
	fn readdir (&mut self, _req: &Request, ino: u64, _fh: u64, offset: u64, mut buffer: DirBuffer) -> FuseResult<DirBuffer> {
		if ino == 1 {
			if offset == 0 {
