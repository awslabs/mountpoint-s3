...
mod test {
    ...
    fn reply_entry() {
        ...
        let attr = FileAttr { ino: 0x11, size: 0x22, blocks: 0x33, atime: time, mtime: time, ctime: time, crtime: time,
            kind: FileType::RegularFile, perm: 0o644, nlink: 0x55, uid: 0x66, gid: 0x77, rdev: 0x88, flags: 0x99, blksize: 0xbb, padding: 0xcc };
    ...
    fn reply_create() {
        ...
        if cfg!(feature = "abi-7-9") {
            ...
            expected[1].splice(insert_at..insert_at, vec![0xdd, 0x00, 0x00, 0x00, 0xee, 0x00, 0x00, 0x00]);
        }
        expected[0][0] = (expected[0].len() + expected[1].len()) as u8;
        ...
        let ttl = Duration::new(0x8765, 0x4321);
        let attr = FileAttr { ino: 0x11, size: 0x22, blocks: 0x33, atime: time, mtime: time, ctime: time, crtime: time,
            kind: FileType::RegularFile, perm: 0o644, nlink: 0x55, uid: 0x66, gid: 0x77, rdev: 0x88, flags: 0x99, blksize: 0xdd, padding: 0xee };
