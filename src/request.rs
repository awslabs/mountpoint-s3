...
impl<'a> Request<'a> {
	...
	pub fn new (buffer: &'a [u8]) -> Option<Request<'a>> {
	...
	pub fn dispatch<FS: Filesystem> (&self, se: &mut Session<FS>) {
		...
		match opcode {
			...
			FUSE_LOOKUP => {
				...
				self.reply(&se.ch, se.filesystem.lookup(self.header.nodeid, &name));
			},
			FUSE_FORGET => {
			...
			FUSE_MKNOD => {
				...
				self.reply(&se.ch, se.filesystem.mknod(self.header.nodeid, &name, arg.mode, arg.rdev));
			},
			FUSE_MKDIR => {
				...
				self.reply(&se.ch, se.filesystem.mkdir(self.header.nodeid, &name, arg.mode));
			},
			FUSE_UNLINK => {
				let name = data.fetch_path();
				debug!("UNLINK({:u}) parent {:#018x}, name {}", self.header.unique, self.header.nodeid, name.display());
				self.reply(&se.ch, se.filesystem.unlink(self.header.nodeid, &name));
			},
			FUSE_RMDIR => {
				let name = data.fetch_path();
				debug!("RMDIR({:u}) parent {:#018x}, name {}", self.header.unique, self.header.nodeid, name.display());
				self.reply(&se.ch, se.filesystem.rmdir(self.header.nodeid, &name));
			},
			FUSE_SYMLINK => {
				let name = data.fetch_path();
				...
				self.reply(&se.ch, se.filesystem.symlink(self.header.nodeid, &name, &link));
			},
			FUSE_RENAME => {
				...
				self.reply(&se.ch, se.filesystem.rename(self.header.nodeid, &name, arg.newdir, &newname));
			},
			FUSE_LINK => {
				...
				self.reply(&se.ch, se.filesystem.link(arg.oldnodeid, self.header.nodeid, &newname));
			},
			FUSE_OPEN => {
				let arg: &fuse_open_in = data.fetch();
				debug!("OPEN({:u}) ino {:#018x}, flags {:#x}", self.header.unique, self.header.nodeid, arg.flags);
				self.reply(&se.ch, se.filesystem.open(self.header.nodeid, arg.flags as uint));
			},
			FUSE_READ => {
				...
				self.reply(&se.ch, se.filesystem.read(self.header.nodeid, arg.fh, arg.offset, arg.size as uint));
			},
			FUSE_WRITE => {
				...
				self.reply(&se.ch, se.filesystem.write(self.header.nodeid, arg.fh, arg.offset, data, arg.write_flags as uint).and_then(|written| {
			...
			FUSE_FLUSH => {
				...
				self.reply(&se.ch, se.filesystem.flush(self.header.nodeid, arg.fh, arg.lock_owner));
			},
			FUSE_RELEASE => {
				...
				self.reply(&se.ch, se.filesystem.release(self.header.nodeid, arg.fh, arg.flags as uint, arg.lock_owner, flush));
			},
			FUSE_FSYNC => {
				...
				self.reply(&se.ch, se.filesystem.fsync(self.header.nodeid, arg.fh, datasync));
			},
			FUSE_OPENDIR => {
				let arg: &fuse_open_in = data.fetch();
				debug!("OPENDIR({:u}) ino {:#018x}, flags {:#x}", self.header.unique, self.header.nodeid, arg.flags);
				self.reply(&se.ch, se.filesystem.opendir(self.header.nodeid, arg.flags as uint));
			},
			FUSE_READDIR => {
				let arg: &fuse_read_in = data.fetch();
				debug!("READDIR({:u}) ino {:#018x}, fh {:u}, offset {:u}, size {:u}", self.header.unique, self.header.nodeid, arg.fh, arg.offset, arg.size);
				self.reply(&se.ch, se.filesystem.readdir(self.header.nodeid, arg.fh, arg.offset, DirBuffer::new(arg.size as uint)));
			},
			FUSE_RELEASEDIR => {
				let arg: &fuse_release_in = data.fetch();
				...
				self.reply(&se.ch, se.filesystem.releasedir(self.header.nodeid, arg.fh, arg.flags as uint));
			},
			FUSE_FSYNCDIR => {
				let arg: &fuse_fsync_in = data.fetch();
				let datasync = match arg.fsync_flags & 1 { 0 => false, _ => true };
				debug!("FSYNCDIR({:u}) ino {:#018x}, fh {:u}, flags {:#x}", self.header.unique, self.header.nodeid, arg.fh, arg.fsync_flags);
				self.reply(&se.ch, se.filesystem.fsyncdir(self.header.nodeid, arg.fh, datasync));
			},
			FUSE_STATFS => {
				debug!("STATFS({:u}) ino {:#018x}", self.header.unique, self.header.nodeid);
				self.reply(&se.ch, se.filesystem.statfs(self.header.nodeid));
			},
			FUSE_SETXATTR => {
				...
				self.reply(&se.ch, se.filesystem.setxattr(self.header.nodeid, name, value, arg.flags as uint, get_position(arg)));
			},
			FUSE_GETXATTR => {
			...
			FUSE_GETLK => {
				...
				self.reply(&se.ch, se.filesystem.getlk(self.header.nodeid, arg.fh, arg.owner, &arg.lk));
			},
			FUSE_SETLK | FUSE_SETLKW => {
				...
				self.reply(&se.ch, se.filesystem.setlk(self.header.nodeid, arg.fh, arg.owner, &arg.lk, sleep));
			},
			FUSE_BMAP => {
				...
				self.reply(&se.ch, se.filesystem.bmap(self.header.nodeid, arg.blocksize as uint, arg.block));
			},
			// OS X only
	...
	fn dispatch_macos_only<FS: Filesystem> (&self, opcode: fuse_opcode, se: &mut Session<FS>) {
		...
		match opcode {
			...
			FUSE_GETXTIMES => {
				...
				self.reply(&se.ch, se.filesystem.getxtimes(self.header.nodeid));
			},
			_ => unreachable!(),
		}
	}
