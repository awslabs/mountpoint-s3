	...
	fn dispatch<FS: Filesystem> (&self, se: &mut Session<FS>) {
		...
		match opcode {
			...
			FUSE_LOOKUP => {
				...
				self.reply(se.filesystem.lookup(self, self.header.nodeid, &name));
			},
			FUSE_FORGET => {
			...
			FUSE_MKNOD => {
				...
				self.reply(se.filesystem.mknod(self, self.header.nodeid, &name, arg.mode, arg.rdev));
			},
			FUSE_MKDIR => {
				...
				self.reply(se.filesystem.mkdir(self, self.header.nodeid, &name, arg.mode));
			},
			FUSE_UNLINK => {
				let name = data.fetch_path();
				debug!("UNLINK({:u}) parent {:#018x}, name {}", self.header.unique, self.header.nodeid, name.display());
				self.reply(se.filesystem.unlink(self, self.header.nodeid, &name));
			},
			FUSE_RMDIR => {
				let name = data.fetch_path();
				debug!("RMDIR({:u}) parent {:#018x}, name {}", self.header.unique, self.header.nodeid, name.display());
				self.reply(se.filesystem.rmdir(self, self.header.nodeid, &name));
			},
			FUSE_SYMLINK => {
				let name = data.fetch_path();
				...
				self.reply(se.filesystem.symlink(self, self.header.nodeid, &name, &link));
			},
			FUSE_RENAME => {
				...
				self.reply(se.filesystem.rename(self, self.header.nodeid, &name, arg.newdir, &newname));
			},
			FUSE_LINK => {
				...
				self.reply(se.filesystem.link(self, arg.oldnodeid, self.header.nodeid, &newname));
			},
			FUSE_OPEN => {
				let arg: &fuse_open_in = data.fetch();
				debug!("OPEN({:u}) ino {:#018x}, flags {:#x}", self.header.unique, self.header.nodeid, arg.flags);
				self.reply(se.filesystem.open(self, self.header.nodeid, arg.flags as uint));
			},
			FUSE_READ => {
				...
				self.reply(se.filesystem.read(self, self.header.nodeid, arg.fh, arg.offset, arg.size as uint));
			},
			FUSE_WRITE => {
				...
				self.reply(se.filesystem.write(self, self.header.nodeid, arg.fh, arg.offset, data, arg.write_flags as uint).and_then(|written| {
			...
			FUSE_FLUSH => {
				...
				self.reply(se.filesystem.flush(self, self.header.nodeid, arg.fh, arg.lock_owner));
			},
			FUSE_RELEASE => {
				...
				self.reply(se.filesystem.release(self, self.header.nodeid, arg.fh, arg.flags as uint, arg.lock_owner, flush));
			},
			FUSE_FSYNC => {
				...
				self.reply(se.filesystem.fsync(self, self.header.nodeid, arg.fh, datasync));
			},
			FUSE_OPENDIR => {
				let arg: &fuse_open_in = data.fetch();
				debug!("OPENDIR({:u}) ino {:#018x}, flags {:#x}", self.header.unique, self.header.nodeid, arg.flags);
				self.reply(se.filesystem.opendir(self, self.header.nodeid, arg.flags as uint));
			},
			FUSE_READDIR => {
				let arg: &fuse_read_in = data.fetch();
				debug!("READDIR({:u}) ino {:#018x}, fh {:u}, offset {:u}, size {:u}", self.header.unique, self.header.nodeid, arg.fh, arg.offset, arg.size);
				self.reply(se.filesystem.readdir(self, self.header.nodeid, arg.fh, arg.offset, DirBuffer::new(arg.size as uint)));
			},
			FUSE_RELEASEDIR => {
				let arg: &fuse_release_in = data.fetch();
				...
				self.reply(se.filesystem.releasedir(self, self.header.nodeid, arg.fh, arg.flags as uint));
			},
			FUSE_FSYNCDIR => {
				let arg: &fuse_fsync_in = data.fetch();
				let datasync = match arg.fsync_flags & 1 { 0 => false, _ => true };
				debug!("FSYNCDIR({:u}) ino {:#018x}, fh {:u}, flags {:#x}", self.header.unique, self.header.nodeid, arg.fh, arg.fsync_flags);
				self.reply(se.filesystem.fsyncdir(self, self.header.nodeid, arg.fh, datasync));
			},
			FUSE_STATFS => {
				debug!("STATFS({:u}) ino {:#018x}", self.header.unique, self.header.nodeid);
				self.reply(se.filesystem.statfs(self, self.header.nodeid));
			},
			FUSE_SETXATTR => {
				...
				self.reply(se.filesystem.setxattr(self, self.header.nodeid, name, value, arg.flags as uint, get_position(arg)));
			},
			FUSE_GETXATTR => {
				...
				match se.filesystem.getxattr(self, self.header.nodeid, name) {
			...
			FUSE_LISTXATTR => {
				...
				match se.filesystem.listxattr(self, self.header.nodeid) {
			...
			FUSE_GETLK => {
				...
				self.reply(se.filesystem.getlk(self, self.header.nodeid, arg.fh, arg.owner, &arg.lk));
			},
			FUSE_SETLK | FUSE_SETLKW => {
				...
				self.reply(se.filesystem.setlk(self, self.header.nodeid, arg.fh, arg.owner, &arg.lk, sleep));
			},
			FUSE_BMAP => {
				...
				self.reply(se.filesystem.bmap(self, self.header.nodeid, arg.blocksize as uint, arg.block));
			},
			// OS X only
	...
	fn dispatch_macos_only<FS: Filesystem> (&self, opcode: fuse_opcode, se: &mut Session<FS>) {
		...
		match opcode {
			FUSE_SETVOLNAME => {
				...
				self.reply(se.filesystem.setvolname(self, name));
			},
			FUSE_EXCHANGE => {
				...
				self.reply(se.filesystem.exchange(self, arg.olddir, &oldname, arg.newdir, &newname, arg.options as uint));
			},
			FUSE_GETXTIMES => {
				debug!("GETXTIMES({:u}) ino {:#018x}", self.header.unique, self.header.nodeid);
				self.reply(se.filesystem.getxtimes(self, self.header.nodeid));
			},
			_ => unreachable!(),
		}
	}
