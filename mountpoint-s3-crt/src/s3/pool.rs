...
#[derive(Debug, Clone)]
pub struct CrtBufferPoolFactory {
    pool: Arc<MemoryPoolWrapper>,
    factory: aws_s3_buffer_pool_factory_fn,
}

impl CrtBufferPoolFactory {
    /// Builds a factory for the given pool.
    pub fn new<Pool: MemoryPool>(pool: Pool) -> Self {
        let pool = Arc::new(MemoryPoolWrapper::new(pool));
        Self {
            pool,
            factory: Some(buffer_pool_factory::<Pool>),
        }
    }

    /// Returns the factory callback and user_data pointer to pass to the CRT, and
    /// wrapper to keep ownership of the underlying instance implementing [MemoryPool].
    pub(crate) fn into_inner(
        self,
    ) -> (
        aws_s3_buffer_pool_factory_fn,
        *mut ::libc::c_void,
        Arc<MemoryPoolWrapper>,
    ) {
        (self.factory, self.pool.boxed_ptr, self.pool)
    }
}

unsafe extern "C" fn buffer_pool_factory<Pool: MemoryPool>(
    allocator: *mut aws_allocator,
    _config: aws_s3_buffer_pool_config,
    user_data: *mut libc::c_void,
) -> *mut aws_s3_buffer_pool {
    // SAFETY: `user_data` references a `Box` owned by the `MemoryPoolWrapper` instance.
    let pool = unsafe { &*(user_data as *mut Pool) };
...
/// Holds an instance of a [MemoryPool] implementation, erasing its type.
#[derive(Debug)]
pub(crate) struct MemoryPoolWrapper {
    cleanup: fn(*mut ::libc::c_void),
    boxed_ptr: *mut ::libc::c_void,
}

// SAFETY: `MemoryPoolWrapper` is safe to transfer across threads because it wraps a [MemoryPool] implementation that is [Send].
unsafe impl Send for MemoryPoolWrapper {}
// SAFETY: `MemoryPoolWrapper` is safe to share across threads because it wraps a [MemoryPool] implementation that is [Sync].
unsafe impl Sync for MemoryPoolWrapper {}

impl MemoryPoolWrapper {
    /// Builds an instance wrapping the given pool.
    pub fn new<Pool: MemoryPool>(pool: Pool) -> Self {
        let pool = Box::new(pool);
        let boxed_ptr = Box::leak(pool) as *mut Pool as *mut libc::c_void;
        Self {
            cleanup: memory_pool_cleanup::<Pool>,
            boxed_ptr,
        }
    }
}
...
impl Drop for MemoryPoolWrapper {
    fn drop(&mut self) {
        (self.cleanup)(self.boxed_ptr);
    }
}

fn memory_pool_cleanup<Pool: MemoryPool>(boxed_ptr: *mut libc::c_void) {
    // SAFETY: `boxed_ptr` was leaked in `MemoryPoolWrapper::new`.
    _ = unsafe { Box::from_raw(boxed_ptr as *mut Pool) };
}
...
impl<Pool: MemoryPool> CrtBufferPool<Pool> {
    ...
    fn reserve(&self, size: usize) -> TicketFuture {
        let future = TicketFuture::new(&self.allocator);
    ...
    fn make_ticket(&self, buffer: Pool::Buffer) -> Pin<Box<Ticket<Pool::Buffer>>> {
        ...
        unsafe {
            ...
            ticket_ref.inner.impl_ = ticket_ref as *mut Ticket<Pool::Buffer> as *mut libc::c_void;
            aws_ref_count_init(
...
impl<Buffer: AsMut<[u8]>> Ticket<Buffer> {
    /// Leak a pinned instance and returns a raw pointer.
    ///
    /// # Safety
    /// The returned pointer must eventually be passed to [from_raw] and can
...
unsafe extern "C" fn ticket_claim<Buffer: AsMut<[u8]>>(mut ticket: *mut aws_s3_buffer_ticket) -> aws_byte_buf {
    ...
    let ticket = unsafe { Ticket::<Buffer>::ref_mut_from_raw(&mut ticket) };
...
unsafe extern "C" fn ticket_destroy<Buffer: AsMut<[u8]>>(data: *mut libc::c_void) {
    ...
    // SAFETY: `ticket` was obtained through `Ticket::leak`.
    _ = unsafe { Ticket::<Buffer>::from_raw(ticket) };
}
...
unsafe impl Send for TicketFuture {}

// SAFETY: `aws_future_s3_buffer_ticket` is reference counted and its methods are thread-safe
unsafe impl Sync for TicketFuture {}
...
impl TicketFuture {
    ...
    fn set<Buffer: AsMut<[u8]>>(&self, ticket: Pin<Box<Ticket<Buffer>>>) {
